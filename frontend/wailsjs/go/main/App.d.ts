// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT
import {models} from '../models';
import {main} from '../models';
import {monitoring} from '../models';
import {context} from '../models';
import {database} from '../models';

export function AddProxy(arg1:string,arg2:string,arg3:string,arg4:string,arg5:string):Promise<models.Proxy>;

export function AddProxyWithRateLimit(arg1:string,arg2:string,arg3:string,arg4:string,arg5:string,arg6:number):Promise<models.Proxy>;

export function CancelTask(arg1:string):Promise<void>;

export function ClearLogs():Promise<void>;

export function CompareVisual(arg1:models.DiffRequest):Promise<models.VisualDiff>;

export function CreateAlertRule(arg1:models.AlertRule):Promise<void>;

export function CreateBatch(arg1:Array<models.BatchTaskInput>,arg2:boolean):Promise<Array<models.Task>>;

export function CreateBatchFromFlow(arg1:models.AdvancedBatchInput):Promise<models.BatchGroup>;

export function CreateProxyRoutingPreset(arg1:string,arg2:string,arg3:string,arg4:boolean):Promise<models.ProxyRoutingPreset>;

export function CreateRecordedFlow(arg1:string,arg2:string,arg3:string,arg4:Array<models.RecordedStep>):Promise<models.RecordedFlow>;

export function CreateRepeatedTask(arg1:models.RepeatTaskInput):Promise<models.BatchGroup>;

export function CreateSchedule(arg1:main.ScheduleParams):Promise<models.Schedule>;

export function CreateTask(arg1:main.CreateTaskParams):Promise<models.Task>;

export function CreateTaskFromFlow(arg1:main.CreateTaskFromFlowParams):Promise<models.Task>;

export function CreateVisualBaseline(arg1:string,arg2:string,arg3:string):Promise<models.VisualBaseline>;

export function DeleteAlertRule(arg1:string):Promise<void>;

export function DeleteCaptchaConfig(arg1:string):Promise<void>;

export function DeleteProxy(arg1:string):Promise<void>;

export function DeleteProxyRoutingPreset(arg1:string):Promise<void>;

export function DeleteRecordedFlow(arg1:string):Promise<void>;

export function DeleteSchedule(arg1:string):Promise<void>;

export function DeleteTask(arg1:string):Promise<void>;

export function DeleteVisualBaseline(arg1:string):Promise<void>;

export function ExportBatchLogs(arg1:string):Promise<string>;

export function ExportFlow(arg1:string):Promise<string>;

export function ExportResultsCSV():Promise<string>;

export function ExportResultsJSON():Promise<string>;

export function ExportTask(arg1:string):Promise<string>;

export function ExportTaskLogs(arg1:string):Promise<string>;

export function GetActiveAlerts():Promise<Array<models.AlertFiring>>;

export function GetAlertRule(arg1:string):Promise<models.AlertRule>;

export function GetAuditTrail(arg1:string,arg2:number):Promise<Array<models.TaskLifecycleEvent>>;

export function GetBatchProgress(arg1:string):Promise<models.BatchProgress>;

export function GetCaptchaConfig():Promise<models.CaptchaConfig>;

export function GetEnhancedPrometheusMetrics():Promise<string>;

export function GetLocalProxyGatewayStats():Promise<models.LocalProxyGatewayStats>;

export function GetLogStats():Promise<monitoring.LogStats>;

export function GetLogsByBatchID(arg1:string,arg2:number):Promise<Array<monitoring.LogEntry>>;

export function GetLogsByLevel(arg1:string,arg2:number):Promise<Array<monitoring.LogEntry>>;

export function GetLogsByTaskID(arg1:string,arg2:number):Promise<Array<monitoring.LogEntry>>;

export function GetMonitoringMetrics():Promise<monitoring.RepeatTaskMetrics>;

export function GetPrometheusMetrics():Promise<string>;

export function GetQueueMetrics():Promise<models.QueueMetrics>;

export function GetRecentLogs(arg1:number):Promise<Array<monitoring.LogEntry>>;

export function GetRecordedFlow(arg1:string):Promise<models.RecordedFlow>;

export function GetRunningCount():Promise<number>;

export function GetSchedule(arg1:string):Promise<models.Schedule>;

export function GetSupportedStepActions():Promise<Array<string>>;

export function GetSystemHealth():Promise<monitoring.HealthStatus>;

export function GetTask(arg1:string):Promise<models.Task>;

export function GetTaskStats():Promise<Record<string, number>>;

export function GetVisualDiff(arg1:string):Promise<models.VisualDiff>;

export function ImportFlow(arg1:string):Promise<Array<models.Task>>;

export function ImportFlowWithWarnings(arg1:string):Promise<main.FlowImportResult>;

export function ImportTask(arg1:string):Promise<models.Task>;

export function IsRecording():Promise<boolean>;

export function ListAlertFirings(arg1:string,arg2:number):Promise<Array<models.AlertFiring>>;

export function ListAlertRules():Promise<Array<models.AlertRule>>;

export function ListBatchGroups():Promise<Array<models.BatchGroup>>;

export function ListCaptchaConfigs():Promise<Array<models.CaptchaConfig>>;

export function ListDOMSnapshots(arg1:string):Promise<Array<models.DOMSnapshot>>;

export function ListProxies():Promise<Array<models.Proxy>>;

export function ListProxyCountryStats():Promise<Array<models.ProxyCountryStats>>;

export function ListProxyRoutingPresets():Promise<Array<models.ProxyRoutingPreset>>;

export function ListRecordedFlows():Promise<Array<models.RecordedFlow>>;

export function ListSchedules():Promise<Array<models.Schedule>>;

export function ListTaskEvents(arg1:string):Promise<Array<models.TaskLifecycleEvent>>;

export function ListTasks():Promise<Array<models.Task>>;

export function ListTasksByBatch(arg1:string):Promise<Array<models.Task>>;

export function ListTasksByStatus(arg1:string):Promise<Array<models.Task>>;

export function ListTasksPaginated(arg1:number,arg2:number,arg3:string,arg4:string):Promise<models.PaginatedTasks>;

export function ListVisualBaselines():Promise<Array<models.VisualBaseline>>;

export function ListVisualDiffs(arg1:string):Promise<Array<models.VisualDiff>>;

export function ListVisualDiffsByTask(arg1:string):Promise<Array<models.VisualDiff>>;

export function ListWebSocketLogs(arg1:string):Promise<Array<models.WebSocketLog>>;

export function ParseBatchURLs(arg1:string,arg2:boolean):Promise<Array<string>>;

export function PauseBatch(arg1:string):Promise<void>;

export function PlayRecordedFlow(arg1:string,arg2:string,arg3:boolean,arg4:number,arg5:models.TaskLoggingPolicy):Promise<models.Task>;

export function PurgeOldData(arg1:number):Promise<number>;

export function ResumeBatch(arg1:string):Promise<void>;

export function RetryFailedBatch(arg1:string):Promise<Array<models.Task>>;

export function SaveCaptchaConfig(arg1:string,arg2:string):Promise<models.CaptchaConfig>;

export function SaveDOMSnapshot(arg1:models.DOMSnapshot):Promise<void>;

export function StartAllPending():Promise<void>;

export function StartRecording(arg1:string):Promise<void>;

export function StartTask(arg1:string):Promise<void>;

export function StopRecording():Promise<Array<models.RecordedStep>>;

export function SubmitScheduledTask(arg1:context.Context,arg2:models.Schedule):Promise<void>;

export function TestCaptchaConfig(arg1:string):Promise<number>;

export function ToggleSchedule(arg1:string,arg2:boolean):Promise<void>;

export function UpdateAlertRule(arg1:models.AlertRule):Promise<void>;

export function UpdateRecordedFlow(arg1:models.RecordedFlow):Promise<void>;

export function UpdateSchedule(arg1:string,arg2:main.ScheduleParams,arg3:boolean):Promise<void>;

export function UpdateTask(arg1:string,arg2:database.TaskUpdateParams,arg3:number):Promise<void>;
